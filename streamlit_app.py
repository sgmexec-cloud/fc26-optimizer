...
if st.session_state.scout_report:
    ...
    if st.button("Calculate Perfect Stats", type="primary"):
        with st.spinner("🧠 Running ultra-fast Python math logic... (Takes ~5 seconds)"):
            try:
                playstyles = pd.read_csv("PLAYSTYLES.csv").to_csv(index=False)
                ...
                CRITICAL OPTIMIZATION RULES FOR YOUR PYTHON SCRIPT (TO PREVENT TIMEOUTS):
                1. DO NOT USE PANDAS FILTERING INSIDE YOUR LOOPS! It is too slow. 
                2. At the very start of your code, convert MASTER_COST_DATA into a nested Python dictionary for instant lookups: `cost_dict[attribute_name][level] = cost`.
                3. Load the Archetype's Base and Max stats into dictionaries: `base_stats[attr]` and `max_stats[attr]`.
                ...
                6. To get the upgrade cost, use your dictionary: `cost = cost_dict[attr].get(current_stats[attr] + 1, 9999)`.
                7. DO NOT use print() statements inside the loops. Calculate everything silently.
