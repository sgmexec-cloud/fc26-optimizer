...
if st.session_state.scout_report:
    ...
    if st.button("Calculate Perfect Stats", type="primary"):
        with st.spinner("🧠 Running strict, silent Python math logic... (This takes about 10-15 seconds)"):
            try:
                playstyles = pd.read_csv("PLAYSTYLES.csv").to_csv(index=False)
                ...
                CRITICAL MATH RULES FOR YOUR PYTHON SCRIPT:
                1. Look up the chosen archetype in ALL_ARCHETYPES. 
                2. Set every single attribute to its exact 'Base Value'. (AP Spent so far = 0).
                3. Note the 'Max Value' for every attribute. You MUST NEVER exceed this number.
                4. UPGRADE LOGIC: To upgrade an attribute from Current Level (X) to (X+1), find the row in MASTER_COST_DATA where 'Attribute' matches and 'Level' equals (X+1). Deduct that exact 'Cost' value from the budget. 
                5. Do this point-by-point for Playstyle minimums, then Core attributes, then Secondary, then Tertiary until budget is EXACTLY 0.
                6. TIMEOUT PREVENTION (CRITICAL): DO NOT use print() statements inside any of your loops! Your Python script must run completely silently. Only print() the final data dictionary at the very end of your code so you can read it to write the final card. Ensure your while loops have strict break conditions so they do not loop infinitely.
                ...
                **Weak Foot**
                ...
                # Safely extract the text to avoid the "finish_reason 2" crash if it ever happens again
