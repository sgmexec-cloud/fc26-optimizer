...
if st.session_state.scout_report:
    ...
    if st.button("Calculate Perfect Stats", type="primary"):
        with st.spinner("🧠 Running strict Python math logic... (This takes about 10-15 seconds)"):
            try:
                playstyles = pd.read_csv("PLAYSTYLES.csv").to_csv(index=False)
                ...
                4. UPGRADE LOGIC: To upgrade an attribute from its Current Level (X) to the Next Level (X+1), find the row in MASTER_COST_DATA where 'Attribute' matches and 'Level' equals (X+1). Deduct that exact 'Cost' value from the budget. 
                ...
                **Weak Foot**
                ...
                # THE MAGIC SCRUBBER: This deletes all Python code and logs from the output
